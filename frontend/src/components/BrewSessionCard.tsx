...
const STATUS_STYLES: Record<string, string> = {
  planned: 'bg-accent-primary/20 text-accent-primary border-accent-primary/30',
  ...
  bottled: 'bg-secondary/20 text-secondary border-secondary/30',
    ...
    >
      <div className="flex justify-between items-start mb-3">
        <div className="min-w-0 flex-1">
          ...
          {session.sessionName && (
            <p className="text-sm text-secondary mt-0.5 truncate">{recipeName}</p>
          )}
        </div>
        <span
          className={`ml-3 text-xs font-medium px-2.5 py-1 rounded-full border shrink-0 ${STATUS_STYLES[session.status] || 'bg-secondary/20 text-secondary border-secondary/30'}`}
        >
      ...
      {(session.actualOg || session.actualAbv) && (
        <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-default/50">
          {session.actualOg && (
            <div className="text-center">
              <p className="text-xs text-muted">OG</p>
          ...
          {session.actualAbv && (
            <div className="text-center">
              <p className="text-xs text-muted">ABV</p>
              <p className="text-sm font-semibold text-accent-primary">
